      ...
      >
        <Box flex={1} backgroundColor="surface">
          ...
          <Box
            ...
            padding="xs"
          >
            <TouchableOpacity style={{ flex: 1 }} onPress={() => switchMode('ai')} accessibilityLabel="Modo IA">
              <Box padding="s" borderRadius="s" alignItems="center" style={{ backgroundColor: mode === 'ai' ? theme.colors.white : 'transparent' }}>
                <Text variant="bodySmall" color={mode === 'ai' ? 'primaryDark' : 'textSecondary'} style={{ fontFamily: mode === 'ai' ? 'Poppins_600SemiBold' : 'Poppins_400Regular' }}>
                  IA ✨
                </Text>
              </Box>
            </TouchableOpacity>
            <TouchableOpacity style={{ flex: 1 }} onPress={() => switchMode('manual')} accessibilityLabel="Modo manual">
              <Box padding="s" borderRadius="s" alignItems="center" style={{ backgroundColor: mode === 'manual' ? theme.colors.white : 'transparent' }}>
                <Text variant="bodySmall" color={mode === 'manual' ? 'primaryDark' : 'textSecondary'} style={{ fontFamily: mode === 'manual' ? 'Poppins_600SemiBold' : 'Poppins_400Regular' }}>
                  Manual 📝
                </Text>
              </Box>
            </TouchableOpacity>
          ...
          >
            ...
            {mode === 'ai' && (
              <>
                <Box backgroundColor="surfaceLight" borderRadius="l" padding="m" marginBottom="m">
                  <Text variant="label" marginBottom="s">Sobre o que você quer estudar? *</Text>
                  <View style={{ backgroundColor: theme.colors.white, borderRadius: 12, borderWidth: 1.5, borderColor: errors.prompt ? theme.colors.error : theme.colors.border, padding: 12 }}>
                    <TextInput
                      value={prompt}
                      ...
                      style={{ fontFamily: 'Poppins_400Regular', fontSize: 15, color: theme.colors.textPrimary, minHeight: 120 }}
                      accessibilityLabel="Tema para gerar flashcards"
                    />
                  </View>
                  {errors.prompt && <Text variant="caption" color="error" marginTop="xs">{errors.prompt}</Text>}
                  <Text variant="caption" color="textSecondary" marginTop="xs">{prompt.length} / mínimo 20 caracteres</Text>
                </Box>

                <Box backgroundColor="surfaceLight" borderRadius="l" padding="m" marginBottom="l">
                  <Text variant="label" marginBottom="s">Quantidade de Flashcards: {cardCount}</Text>
                  ...
                  <Text variant="caption" color="textSecondary" marginTop="xs">Mínimo: 5 | Máximo: 30</Text>
                </Box>

                <Button label={isGenerating ? loadingMessage : 'Gerar Flashcards com IA ✨'} onPress={handleGenerate} isLoading={isGenerating} fullWidth />
            ...
            {mode === 'manual' && (
              <>
                ...
                {manualCards.map((card, idx) => (
                  <Box key={card.id} backgroundColor="surfaceLight" borderRadius="l" padding="m" marginBottom="m">
                    <Box flexDirection="row" alignItems="center" justifyContent="space-between" marginBottom="s">
                      ...
                      {manualCards.length > 1 && (
                        <TouchableOpacity onPress={() => removeCard(card.id)} accessibilityLabel={`Remover card ${idx + 1}`} hitSlop={{ top: 8, bottom: 8, left: 8, right: 8 }}>
                    ...
                    <TextInput
                      ...
                      multiline
                      textAlignVertical="top"
                      style={{ fontFamily: 'Poppins_400Regular', fontSize: 14, color: theme.colors.textPrimary, backgroundColor: theme.colors.white, borderWidth: 1.5, borderColor: errors[`card_${card.id}_question`] ? theme.colors.error : theme.colors.border, borderRadius: 10, padding: 12, minHeight: 80, marginBottom: 4 }}
                    />
                    ...
                    <TextInput
                      ...
                      multiline
                      textAlignVertical="top"
                      style={{ fontFamily: 'Poppins_400Regular', fontSize: 14, color: theme.colors.textPrimary, backgroundColor: theme.colors.white, borderWidth: 1.5, borderColor: errors[`card_${card.id}_answer`] ? theme.colors.error : theme.colors.border, borderRadius: 10, padding: 12, minHeight: 80, marginBottom: 4 }}
                    />
                ...
                <TouchableOpacity onPress={addCard} accessibilityLabel="Adicionar card">
                  <Box borderRadius="l" padding="m" alignItems="center" justifyContent="center" flexDirection="row" style={{ gap: 8, borderWidth: 2, borderColor: theme.colors.primaryDark, borderStyle: 'dashed' }}>
                    <Plus size={20} color={theme.colors.primaryDark} />
                    <Text variant="bodySmall" color="primaryDark" style={{ fontFamily: 'Poppins_600SemiBold' }}>Adicionar Card</Text>
                  </Box>
                </TouchableOpacity>
